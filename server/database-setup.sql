...
-- ============================================================
-- TABLE: iot_audit_log
-- Logs all ESP32 keypad verification attempts and PIR updates
-- ============================================================
CREATE TABLE `iot_audit_log` (
  `id` INT AUTO_INCREMENT PRIMARY KEY,
  `event_type` ENUM('verification', 'occupancy_update', 'error') NOT NULL,
...
-- ============================================================
-- NOTE: Sample bookings will be created after demo users are seeded
-- Run: node seed-demo-data.js to create demo users and sample bookings
-- ============================================================

-- ============================================================
-- VERIFY TABLES CREATED
-- ============================================================
-- Check that all tables were created by querying table information:
-- SELECT TABLE_NAME FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = 'pickleball_db';

-- ============================================================
-- NOTES FOR QUERIES USED IN BACKEND:
-- ============================================================
--
-- 1. FETCH BOOKINGS WITH USER NAMES (JOIN Query):
--    SELECT b.id, b.user_id, u.name AS user_name, u.email AS user_email,
--           b.date, b.start_hour, b.duration, b.time_slot, b.access_code,
--           b.court_number, b.status, b.created_at
--    FROM bookings b
--    LEFT JOIN users u ON b.user_id = u.id
--    ORDER BY b.date DESC;
--
--    ✓ User names will update automatically if a user edits their profile
--
-- 2. VALIDATE TIME RANGE OVERLAP:
--    SELECT start_hour, duration FROM bookings 
--    WHERE court_number = ? AND date = ? AND status != 'cancelled'
--
--    Then check: if new_start <= booked_end AND new_end >= booked_start → OVERLAP
--
-- 3. VERIFY ACCESS CODE (for ESP32):
--    SELECT start_hour, duration FROM bookings
--    WHERE access_code = ? AND date = TODAY() AND status = 'confirmed'
--
--    Grant access if: current_hour >= start_hour AND current_hour < (start_hour + duration)
--
-- ============================================================
